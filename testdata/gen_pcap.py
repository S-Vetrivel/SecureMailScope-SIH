...
def create_pcap(filename):
    with open(filename, 'wb') as f:
        ...
        # 1. SMTPS Session 1 (Port 465 - TLS 1.3)
        # SYN
        ...
        packets.append(make_pkt('192.168.1.50', '198.51.108.25', 54321, 465, 1001, 2001, 0x10)) # ACK
        
        # TLS ClientHello (TLS 1.2 record header containing TLS 1.3 ClientHello extensions)
        client_hello_payload = bytes.fromhex(
            ...
            "001300040002001d" # Supported group x25519 (0x001d) -> Forward Secrecy!
        )
        ...
        server_hello_payload = bytes.fromhex(
            ...
            "002b00020304" # Supported version TLS 1.3 chosen
        ...
        # 2. STARTTLS Session 2 (Port 25 - Plaintext -> STARTTLS upgrade)
        ...
        packets.append(make_pkt('192.168.1.55', '198.51.100.26', 54322, 25, 3001, 4001, 0x18, b"220 mail.secure.org ESMTP\r\n"))
        packets.append(make_pkt('192.168.1.55', '198.51.100.26', 54322, 25, 3001, 4025, 0x18, b"EHLO client.org\r\n"))
        packets.append(make_pkt('198.51.100.26', '192.168.1.55', 25, 54322, 4025, 3018, 0x18, b"250-mail.secure.org\r\n250 STARTTLS\r\n"))
        packets.append(make_pkt('192.168.1.55', '198.51.100.26', 54322, 25, 3018, 4060, 0x18, b"STARTTLS\r\n"))
        packets.append(make_pkt('198.51.100.26', '192.168.1.55', 25, 54322, 4060, 3028, 0x18, b"220 2.0.0 Ready to start TLS\r\n"))
